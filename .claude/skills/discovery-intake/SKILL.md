...
**Gate compilation (native enforcement):** for each approved `gate_protected_areas` pattern, add `"Edit(<pattern>)"` and `"Write(<pattern>)"` to `permissions.ask` in `.claude/settings.json` (merge — never remove existing rules). `project.yml` stays the declarative source with the `reason`; `settings.json` is the enforced runtime form. Patterns already present are skipped.
...
3. **Apply** — on `y`:
   ...
   - Compile `gate_protected_areas` into `permissions.ask` rules in
     `.claude/settings.json` (see Gate compilation above).
